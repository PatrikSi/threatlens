...
import type { InvestigationDetailController, InvestigationMutationOperation } from './useInvestigationDetail'
        ...
        <div className="mt-2 flex min-w-0 flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              <h1 className="min-w-0 break-words font-display text-xl leading-tight sm:text-2xl">{detail.title}</h1>
      ...
      <div className="space-y-2 px-3 pt-3 sm:px-4">
        {controller.detailQuery.isError && (
          <InvestigationRefreshWarning onRetry={() => void controller.detailQuery.refetch()}>
            {resolveApiErrorMessage(controller.detailQuery.error, 'Investigation could not be refreshed')} The last loaded workspace remains visible.
          </InvestigationRefreshWarning>
        )}
        {controller.conflictNotice && (
          <div role="alert" className="flex flex-wrap items-center justify-between gap-2 rounded border border-amber-300/70 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:border-amber-700/50 dark:bg-amber-950/30 dark:text-amber-100">
            ...
            <button type="button" className="min-h-11 rounded border border-amber-400 px-3 py-2 font-semibold md:min-h-0 md:py-1 dark:border-amber-700" onClick={() => void controller.refreshLatest()}>
              Refresh latest
            </button>
          </div>
        )}
        {controller.mutation.isError && !isInvestigationVersionConflict(controller.mutation.error) && (
          <InvestigationInlineMessage tone="error">
            {resolveApiErrorMessage(controller.mutation.error, mutationFallback(controller.mutation.variables), {
              retryGuidance: 'Review the submitted values and try again. Unsaved input has been preserved.',
            })}
          </InvestigationInlineMessage>
        )}
        {controller.successNotice && <InvestigationInlineMessage tone="success">{controller.successNotice}</InvestigationInlineMessage>}
        {controller.access.readOnlyReason && <InvestigationInlineMessage tone="info">{controller.access.readOnlyReason}</InvestigationInlineMessage>}
      </div>

      <main className="min-w-0 px-3 py-3 sm:px-4 sm:py-4">
        {controller.activeTab === 'overview' && <InvestigationOverviewPanel controller={controller} />}
        {controller.activeTab === 'members' && <InvestigationMembersPanel controller={controller} />}
        {controller.activeTab === 'evidence' && <InvestigationEvidencePanel controller={controller} />}
        {controller.activeTab === 'notes' && <InvestigationNotesPanel controller={controller} />}
        {controller.activeTab === 'activity' && <InvestigationActivityPanel controller={controller} />}
      </main>
      ...
      <LifecycleDialog
        ...
        onConfirm={() => {
          if (lifecycleConfirmation === 'close') {
            runLifecycleMutation({ kind: 'update', changes: { status: 'closed', disposition: closeDisposition.trim() || null } })
          } else if (lifecycleConfirmation === 'reopen') {
            runLifecycleMutation({ kind: 'update', changes: { status: 'open', disposition: null } })
          } else if (lifecycleConfirmation === 'archive') {
            runLifecycleMutation({ kind: 'update', changes: { status: 'archived' } })
          }
        }}
      />
    </div>
...
function DetailTabs({ controller }: { controller: InvestigationDetailController }) {
  ...
  return (
    <nav aria-label="Investigation workspace" className="mt-3 flex gap-1 overflow-x-auto border-t border-slate/15 pt-3 dark:border-white/10">
      {INVESTIGATION_TABS.map((tab) => (
        ...
        >
          {tab.label}{typeof counts[tab.value] === 'number' ? ` (${counts[tab.value]})` : ''}
        </button>
      ))}
    </nav>
  )
...
function LifecycleActions({
  ...
  if (status === 'archived') {
    return canReopen ? <button type="button" className="min-h-11 rounded border border-slate/30 px-3 py-2 text-sm font-semibold dark:border-white/15" disabled={pending} onClick={() => onConfirm('reopen')}>Reopen investigation</button> : null
  }
  if (!canWrite) return null
  return (
    <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap">
      {status === 'open' && <button type="button" className="min-h-11 rounded border border-slate/30 px-3 py-2 text-sm font-semibold dark:border-white/15" disabled={pending} onClick={() => onUpdateStatus('monitoring')}>Start monitoring</button>}
      {status === 'monitoring' && <button type="button" className="min-h-11 rounded border border-slate/30 px-3 py-2 text-sm font-semibold dark:border-white/15" disabled={pending} onClick={() => onUpdateStatus('open')}>Return to open</button>}
      {status !== 'closed' && <button type="button" className="min-h-11 rounded bg-ink px-3 py-2 text-sm font-semibold text-white dark:bg-cyan dark:text-[#053c2e]" disabled={pending} onClick={() => onConfirm('close')}>Close investigation</button>}
      {status === 'closed' && <button type="button" className="min-h-11 rounded border border-slate/30 px-3 py-2 text-sm font-semibold dark:border-white/15" disabled={pending} onClick={() => onConfirm('reopen')}>Reopen investigation</button>}
      {status === 'closed' && canArchive && <button type="button" className="tl-button-danger col-span-2 min-h-11 rounded px-3 py-2 text-sm font-semibold sm:col-auto" disabled={pending} onClick={() => onConfirm('archive')}>Archive</button>}
    </div>
  )
}
...
function LifecycleDialog({
  ...
  const content = {
    close: { title: 'Close investigation?', description: `Close “${title}” and move it out of active work?`, label: 'Close investigation', tone: 'primary' as const },
    reopen: { title: 'Reopen investigation?', description: `Return “${title}” to active investigation work? Its previous closure disposition will be cleared.`, label: 'Reopen investigation', tone: 'primary' as const },
    archive: { title: 'Archive investigation?', description: `Archive “${title}”? It will become read-only until an owner or editor explicitly reopens it.`, label: 'Archive investigation', tone: 'danger' as const },
  }
  if (!kind) return null
