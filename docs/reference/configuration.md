...
- For a new local installation, `./bootstrap.sh` generates `.env` with fresh random secrets, HTTP-friendly local settings, and one-time admin seeding enabled. It requires Bash and either OpenSSL or Python 3. Bootstrap generates configuration only; run `docker compose pull` and `docker compose up -d --wait` to start the stack, then log in at `http://localhost:3000` with the printed credentials.
