...
def _build_stats_window(days: int) -> StatsWindow:
    ...
    return StatsWindow(
        generated_at=generated_at, start_at=start_at, start_date=window_start_date
    )
...
def get_stats_overview(
    ...
    item_counts_query = select(
        ...
        func.sum(case((Item.status == "content_fetched", 1), else_=0)).label(
            "items_content_fetched"
        ),
        func.sum(case((Item.status == "error", 1), else_=0)).label("items_error"),
        func.sum(
            case(
                (Item.first_seen_at >= window.generated_at - timedelta(hours=24), 1),
                else_=0,
            )
        ).label("items_last_24h"),
        func.sum(
            case(
                (Item.first_seen_at >= window.generated_at - timedelta(days=7), 1),
                else_=0,
            )
        ).label("items_last_7d"),
        func.sum(
            case(
                (Item.first_seen_at >= window.generated_at - timedelta(days=30), 1),
                else_=0,
            )
        ).label("items_last_30d"),
    )
    if item_filters:
    ...
    if item_filters:
        articles_query = articles_query.join(Item, Item.id == Article.item_id).where(
            *item_filters
        )
    ...
    status_rows = db.execute(
        status_query.group_by(Item.status).order_by(func.count().desc())
    ).all()
    status_breakdown = [
        StatusPoint(status=status, count=int(count)) for status, count in status_rows
    ]
    ...
    daily_volume = [
        DailyVolumePoint(date=str(date_key), count=int(count or 0))
        for date_key, count in volume_rows
        if date_key
    ]
    ...
    extraction_success_rate = (
        (items_content_fetched / items_total * 100.0) if items_total else 0.0
    )
    error_rate = (items_error / items_total * 100.0) if items_total else 0.0
    avg_items_per_day_window = (
        (sum(point.count for point in daily_volume) / days) if days else 0.0
    )
...
def get_feed_timeseries(
    ...
    feed_rows = db.execute(
        select(Feed.id, Feed.name).where(Feed.id.in_(target_feed_ids))
    ).all()
    ...
    counts_by_feed_and_date: dict[uuid.UUID, dict[str, int]] = {
        feed_id: {} for feed_id in target_feed_ids
    }
    ...
    date_axis = [
        (window.start_date + timedelta(days=offset)).isoformat()
    ...
    series = [
        FeedTimeSeriesSeries(
            ...
            points=[
                FeedTimeSeriesPoint(
                    date=date_key,
                    count=counts_by_feed_and_date.get(feed_id, {}).get(date_key, 0),
                )
...
def get_activity_heatmap(
    ...
    bucket_labels = (
        [f"{hour:02d}:00" for hour in range(24)] if bucket_unit == "hour" else ["Daily"]
    )
    ...
    rows = [
        ActivityHeatmapDayRow(day=day_key, counts=counts_by_day[day_key])
        for day_key in day_axis
    ]
...
def get_signal_radar(
    ...
    query = (
        select(
            ItemClassification.primary_category, func.count(ItemClassification.item_id)
        )
    ...
    categories = [
        category for category in CLASSIFICATION_CATEGORIES if category != "multi"
    ] + ["multi"]
    counts_by_category = {
        category: raw_counts.get(category, 0) for category in categories
    }
...
def _load_top_domains(
    ...
    *,
    ...
    return [
        DomainPoint(domain=domain, count=int(count or 0))
        for domain, count in db.execute(query)
        if domain
    ]
