...
def list_feeds(db: Session = Depends(get_db), _=Depends(get_current_user)):
    feeds = db.scalars(select(Feed).order_by(Feed.created_at.desc())).all()
    return list(feeds)


@router.post("", response_model=FeedResponse, status_code=status.HTTP_201_CREATED)
def create_feed(payload: FeedCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def update_feed(feed_id: uuid.UUID, payload: FeedUpdate, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def delete_feed(feed_id: uuid.UUID, db: Session = Depends(get_db), _=Depends(get_current_user)):
    feed = db.scalar(select(Feed).where(Feed.id == feed_id))
    if feed is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Feed not found")
...
def refresh_feed(feed_id: uuid.UUID, db: Session = Depends(get_db), _=Depends(get_current_user)):
