...
class OIDCProvider(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_oidc_providers_access_policy_generation",
        ),
    )
    ...
    system_key: Mapped[str] = mapped_column(
        String(64), nullable=False, unique=True, default="primary"
    )
    ...
    role_claim: Mapped[str] = mapped_column(
        String(255), nullable=False, default="groups", server_default="groups"
    )
    role_mappings_json: Mapped[list[dict[str, str]]] = mapped_column(
        JSON, nullable=False, default=list
    )
    default_role: Mapped[str] = mapped_column(
        String(32), nullable=False, default=ROLE_VIEWER, server_default=ROLE_VIEWER
    )
    ...
    oidc_access_policy_generation: Mapped[int] = mapped_column(
...
class ExternalIdentity(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "provider_id", "user_id", name="uq_external_identities_provider_user"
        ),
        CheckConstraint(
            ...
            name="ck_external_identities_role_sync_provenance",
        ),
    )
    ...
    role_sync_provenance: Mapped[str | None] = mapped_column(String(16), nullable=True)
    role_sync_previous_role: Mapped[str | None] = mapped_column(
        String(32), nullable=True
    )
    role_sync_applied_role: Mapped[str | None] = mapped_column(
        String(32), nullable=True
    )
    role_sync_updated_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
