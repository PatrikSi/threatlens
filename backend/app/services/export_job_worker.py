...
from app.services.export_query import (
    ExportAuthorizationChangedError, ExportSnapshotChangedError, ExportTextProjection,
    build_export_query_context, iter_export_records, load_export_item_ids,
)
...
def _owned_job(db, job_id, token):
    ...
    if (job is None or job.status != "running" or job.claim_token != token
            or job.lease_expires_at <= now or job.expires_at <= now):
        raise ExportJobInterrupted("Export job claim is no longer current")
    return job


def claim_export_job(job_id):
    settings = get_settings()
    with session_module.SessionLocal() as db:
        job = db.scalar(select(ExportJob).where(ExportJob.id == job_id).with_for_update(skip_locked=True))
        now = datetime.now(timezone.utc)
        ...
        job.lease_expires_at = now + timedelta(seconds=settings.export_job_lease_seconds)
        job.started_at = now
        job.attempts += 1
        job.error_code = None
...
@contextmanager
def _renewing_claim(job_id, token):
    ...
    thread = threading.Thread(target=renew, name=f"export-job-lease:{job_id}", daemon=True)
    thread.start()
    try:
        yield lost
    finally:
...
def execute_export_job(job_id: uuid.UUID):
    ...
    try:
        with _renewing_claim(job_id, token) as lost:
            ...
            with session_module.SessionLocal() as db:
                ...
                with acquire_export_lock(
                    settings=settings, principal_type=job.principal_type, principal_id=job.principal_id,
                ):
        ...
        logger.warning("export_job_attempt_failed job_id=%s error_type=%s", job_id, type(exc).__name__)
...
def _checkpoint(job_id, token, started, lost):
    last_checked = -float("inf")

    def check(*, completed=None, force=False):
...
def _generate(db, job, token, checkpoint, scratch):
    settings = get_settings()
    ...
    context = build_export_query_context(
        ...
        filters=payload.filters, data_access=access,
    )
    ...
    with session_module.SessionLocal() as state_db:
        ...
        state_db.commit()
    needs_text = {
        "stix": False, "csv": payload.options.csv_include_article_text,
    ...
    records = iter_export_records(
        db, item_ids=item_ids, context=context, include_iocs=payload.options.include_iocs,
    ...
    return generate_export_artifact(
        checked_records(), item_count=len(item_ids), export_format=payload.format,
        filters=payload.filters, options=payload.options,
        max_uncompressed_bytes=settings.export_max_uncompressed_bytes,
        artifact_directory=scratch,
    )


def _store_artifact(job_id, token, artifact, checkpoint):
    ...
    with artifact.path.open("rb") as source:
        ...
        while raw := source.read(CHUNK_BYTES):
            ...
            with session_module.SessionLocal() as db:
                ...
                db.add(ExportJobChunk(job_id=job_id, position=position, ciphertext=ciphertext))
                db.commit()
            position += 1


def _publish(job_id, token, artifact):
    with session_module.SessionLocal() as db:
        job = _owned_job(db, job_id, token)
...
def _settle_failure(job_id, token, exc):
    with session_module.SessionLocal() as db:
        try:
            job = _owned_job(db, job_id, token)
        except ExportJobInterrupted:
            return
        if isinstance(exc, (ExportJobAccessDenied, AuthorizationStateUnavailable, DataPolicyError, ExportAuthorizationChangedError)):
        ...
        retry = isinstance(exc, (ExportAlreadyRunningError, ExportLockUnavailableError, OSError))
        if retry and (isinstance(exc, ExportAlreadyRunningError) or job.attempts < get_settings().export_job_max_attempts):
            db.execute(delete(ExportJobChunk).where(ExportJobChunk.job_id == job.id))
            job.status = "queued"
            job.claim_token = None
            job.lease_expires_at = None
            job.next_attempt_at = datetime.now(timezone.utc) + timedelta(seconds=min(60, 10 * job.attempts))
