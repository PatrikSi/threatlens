...
def load_oidc_metadata(provider: OIDCProvider, *, force: bool = False) -> OIDCMetadata:
    ...
    discovery_url = (
        f"{provider.issuer_url.rstrip('/')}/.well-known/openid-configuration"
    )
    ...
    with _metadata_cache_lock:
        _metadata_cache[cache_key] = (
            now + settings.oidc_metadata_cache_seconds,
            config_marker,
            metadata,
        )
    return metadata
    ...
    *,
    ...
    with OAuth2Client(
        ...
        if max_age_seconds is not None:
            extra_parameters["max_age"] = max(0, int(max_age_seconds))
        if prompt:
            extra_parameters["prompt"] = prompt
    ...
    if returned_state != state:
        raise OIDCProtocolError(
            "OIDC client returned an unexpected authorization state"
        )
...
def exchange_oidc_code(
    ...
    *,
    ...
    if provider.client_auth_method == "client_secret_basic":
        if not client_secret:
            raise OIDCConfigurationError(
                "OIDC client secret is required for client_secret_basic"
            )
        auth = httpx.BasicAuth(provider.client_id, client_secret)
    elif provider.client_auth_method == "client_secret_post":
        if not client_secret:
            raise OIDCConfigurationError(
                "OIDC client secret is required for client_secret_post"
            )
    ...
    if "error" in token:
        ...
        raise OIDCProtocolError(
            f"OIDC token endpoint rejected the authorization code ({error_code})"
        )
...
def validate_oidc_token_claims(
    ...
    *,
    ...
    try:
        try:
            parsed_token = jose_jwt.decode(
                token["id_token"], key_set, algorithms=algorithms
            )
...
def oidc_identity_authenticated_at(claims: OIDCClaims) -> datetime | None:
    auth_time = claims.claims.get("auth_time")
    if isinstance(auth_time, bool) or not isinstance(auth_time, (int, float)):
        return None
    try:
        authenticated_at = datetime.fromtimestamp(float(auth_time), tz=timezone.utc)
    except (OSError, OverflowError, ValueError):
        return None
    return authenticated_at


def oidc_external_mfa_asserted(claims: OIDCClaims) -> bool:
    amr = claims.claims.get("amr")
    return (
        isinstance(amr, list)
        and all(isinstance(value, str) for value in amr)
        and "mfa" in {value.strip().lower() for value in amr}
    )


def validate_oidc_reauthentication(
    claims: OIDCClaims,
    *,
    earliest_auth_time: int,
    now: datetime | None = None,
) -> datetime:
    authenticated_at = oidc_identity_authenticated_at(claims)
    if authenticated_at is None:
        raise OIDCProtocolError(
            "OIDC reauthentication did not include a valid auth_time claim"
        )
    current_time = now or datetime.now(timezone.utc)
    if authenticated_at > current_time.replace(microsecond=0) + timedelta(seconds=60):
        raise OIDCProtocolError("OIDC auth_time is unexpectedly in the future")
    if authenticated_at.timestamp() < earliest_auth_time - 60:
        raise OIDCProtocolError(
            "OIDC authentication is older than the requested reauthentication"
        )
    return authenticated_at
...
def _parse_metadata(provider: OIDCProvider, raw: dict[str, Any]) -> OIDCMetadata:
    ...
    for field_name in ("authorization_endpoint", "token_endpoint", "jwks_uri"):
        ...
        validate_oidc_endpoint_url(
            value, field_name=field_name.replace("_", " ").title()
        )
    ...
    auth_methods = raw.get(
        "token_endpoint_auth_methods_supported", ["client_secret_basic"]
    )
    if not isinstance(auth_methods, list) or not all(
        isinstance(value, str) for value in auth_methods
    ):
        raise OIDCProtocolError(
            "OIDC discovery token authentication methods are invalid"
        )
    signing_algorithms = raw.get("id_token_signing_alg_values_supported", ["RS256"])
    if not isinstance(signing_algorithms, list) or not all(
        isinstance(value, str) for value in signing_algorithms
    ):
...
def _allowed_signing_algorithms(metadata: OIDCMetadata) -> tuple[str, ...]:
    ...
    if not allowed:
        raise OIDCProtocolError(
            "OIDC provider does not advertise a supported asymmetric ID token signing algorithm"
        )
    return allowed


def _ensure_auth_method_supported(
    provider: OIDCProvider, metadata: OIDCMetadata
) -> None:
    if (
        provider.client_auth_method
        not in metadata.token_endpoint_auth_methods_supported
    ):
        raise OIDCConfigurationError(
...
def _fetch_json(
    ...
    *,
    ...
    try:
        ...
        with build_safe_http_client(
            ...
            try:
                if response.is_redirect:
                    raise OIDCProtocolError(
                        "OIDC endpoint returned an unexpected redirect"
                    )
                if response.status_code < 200 or response.status_code >= 300:
                    raise OIDCProtocolError(
                        f"OIDC endpoint returned HTTP {response.status_code}"
                    )
        ...
        raise OIDCProtocolError(
            "OIDC endpoint is blocked by outbound network policy"
        ) from exc
    except RedirectError as exc:
        raise OIDCProtocolError(
            "OIDC discovery redirect could not be followed safely"
        ) from exc
...
def _ensure_oidc_runtime_fetchable_url(
...
def _exception_chain_contains(
    exc: BaseException, error_type: type[BaseException]
) -> bool:
